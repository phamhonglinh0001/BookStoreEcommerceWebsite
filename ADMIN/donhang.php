...
if(isset($_SESSION["MSNV"])){
    ...
    $_SESSION["err"] = "Bạn hãy đăng nhập để vào quản lý đơn hàng";
                        ...
                        if(count($thongtin)>0){
                            foreach($thongtin as $x){
                                ...
                                echo '<td class="text-center">'.$x["SoDonDH"].'<br><br><a href="chitiet_donhang.php?SoDonDH='.$x["SoDonDH"].'">Xem chi tiết</a></td>';
                                    ...
                                    echo '<td class="text-center">'.$x["SoDonDH"];
                                }
                                echo
                                '
                                <td>
                                ...
                                if($x["TrangThaiDH"]=="Chờ xác nhận"){
                                    ...
                                    '
                                    <td class="text-center">
                                        <form action="xulydonhang.php?cn=xacnhan&id='.$x["SoDonDH"].'" method="post">
                                            <input required class="form-control" name="giamgia" placeholder="Nhập giảm giá" type="number">
                                            <br>
                                            <button style="width:90%" type="submit" class="btn btn-primary">Xác nhận ĐH</button>
                                            <br><br>
                                            <a style="width:90%" href="xulydonhang.php?cn=huy&id='.$x["SoDonDH"].'" class="btn btn-danger">Hủy đơn</a>
                                        </form>
                                    </td>
                                    '; 
                                }else if($x["TrangThaiDH"]=="Đang đóng gói"){

                                    echo
                                    '
                                    <td class="text-center">
                                        <a class="btn btn-danger" href="xulydonhang.php?cn=danggiaohang&id='.$x["SoDonDH"].'">Đóng gói xong</a>
                                    </td>
                                    '; 
                                }else if($x["TrangThaiDH"]=="Đang giao hàng"){

                                    echo
                                    '
                                    <td class="text-center">
                                        <a class="btn btn-danger" href="xulydonhang.php?cn=dagiaohang&id='.$x["SoDonDH"].'">Đã giao hàng</a>
                                    </td>
                                    '; 
                                }else{
                                    echo '<td></td>';
